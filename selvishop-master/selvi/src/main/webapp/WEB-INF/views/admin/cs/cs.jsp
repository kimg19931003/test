...
<link href="/resources/css/admin/cs/cs.css?i=<%=i_order_new%>"
	rel="stylesheet" type="text/css" />
<link href="/resources/css/common/datatable.css?i=<%=i_order_new%>"
	rel="stylesheet" type="text/css" />
<script src="/resources/js/common/datatable.js?i=<%=i_order_new%>"></script>
<script type="text/javascript"
	src="/resources/js/sweetalert/sweetalert.js?i=<%=i_order_new%>"></script>
</head>
...
<body>
	...
	<div id="customer_form">
		...
		<div class="detail_search">
			...
			<select data-placeholder="상품상태 선택" placeholder="상품상태 선택"
				class="state select" multiple tabindex="3">
			...
			<select data-placeholder="결제 방법" placeholder="상품상태 선택"
				class="pay_method select" multiple tabindex="3">
			...
			<select data-placeholder="주문일" placeholder="상품상태 선택"
				class="order_date select">
		...
		<div id="customer_title_line">
			...
			<div class="field_header_contents shop-boards">
				<div class="button-wrap hover-trans tooltip">
					<button class="btn excelExportButton btn-gray"
						id="downloadAllboardsBtn">
		...
		<table id="customer_table">
			...
			<thead>
				<th><input type='checkbox' id="order_radio" /><label
					for="order_radio" class="customer_radio"></th>
				<th>상품</th>
				<th>구매자</th>
				<th>상품 금액</th>
				...
				<c:choose>
					<c:when test="${fn:length(list) > 0}">
						<c:forEach items="${list}" var="row" varStatus="g">
							...
							<c:set var="order_prdt_option_name"
								value="${row.order_prdt_option_name }" />

							<tr data-id="${row.order_idx_pk}">
								<td><input type='checkbox' id="order_radio${g.index }" /><label
									for="order_radio${g.index }" class="customer_radio"></label></td>


								<td><c:set var="thumbnail_arr"
										value="${fn:split(row.order_prdt_thumbnail,'§')}" /> <c:forEach
										var="thumbnail" items="${thumbnail_arr}" varStatus="g">

										<c:if test="${g.index eq 0}">
											<img style="width: 125px;"
												src="/resources/img/admin/product/${thumbnail}" />
										</c:if>
								...
								<td>${fn:substring(row.order_buy_time,0,4)}년
									${fn:substring(row.order_buy_time,4,6)}월
									${fn:substring(row.order_buy_time,6,8)}일
									${fn:substring(row.order_buy_time,8,10)}:${fn:substring(row.order_buy_time,10,12)}</td>
	...
	<div id="customer_info_form">
		...
		<div id="customer_info_header">
			<div class="customer_info_header_form">
				<div class="customer_back_img">
					<img src="/resources/img/customer_back.png" />
				</div>
		...
		<div class="customer_info_form">
			...
			<table class="customer_info_product">
				...
				<tbody>
					...
					<tr>
						<td class='customer_info_product_img'><img
							src='//m.selvishop.kr/web/product/big/201901/a19fc8a525b36db0f9619ea8690b3a20.jpg' />
						</td>
			...
			<div class="customer_initial_info">
				...
				<div class="initial_date">
					<span>가입일</span> <span class="order_buy_time"></span>
				</div>

				<div class="initial_id">
					<span>아이디</span> <span class="order_id">sasaa3865</span>
				</div>

				<div class="initial_email">
					<span>이메일</span> <span class="order_pay_email"></span>
				</div>

				<div class="initial_name">
					<span>이름</span> <span><input class="order_name" type='text'
						value="김근호" /></span>
				</div>

				<div class="initial_phone">
					<span>휴대폰 번호</span> <span> <input type='text' value="" /> -
						<input type='text' value="" /> - <input type='text' value="" />
					</span>
				</div>

				<div class="initial_post">
					<span>우편 번호</span> <span> <input type='text' value=""
						readonly />
						<button>검색하기</button>
					</span>
				</div>
				...
				<div class="initial_addr">
					<span>주소</span> <span> <input type='text' value="" readonly />
						<input type='text' value="" />
					</span>
				</div>

				<div class="initial_birth">
					<span>생년월일</span> <span> <input type='text' value=""
						placeholder="YYYY" /> <input type='text' value="" placeholder="MM"
						class="" /> <input type='text' value="" placeholder="DD"
						class="day" />
					</span>
				</div>

				<div class="initial_gender">
					<span>성별</span> <span> <span><input type='radio' />남자</span>
						<span><input type='radio' />여자</span>
					</span>
				</div>

				<div class="initial_exit">
					<span>탈퇴여부</span> <span> 탈퇴시키기 </span>
				</div>
			...
			<div class="customer_activity_info">
				...
				<div class="activity_login_time">
					<span>최근 로그인</span> <span>2017.12.10 20:41:44</span>
				</div>

				<div class="activity_repair_time">
					<span>최근 수정일</span> <span>2017.12.10 20:41:44</span>
				</div>

				<div class="activity_rate">
					<span>등급</span> <span> <input type='text' value="김근호" />
					</span>
				</div>

				<div class="activity_saving">
					<span>적립금</span> <span>0원</span>
				</div>

				<div class="activity_coupon">
					<span>쿠폰</span> <span>0매</span>
				</div>

				<div class="activity_buy_price">
					<span>구매 금액</span> <span>0원</span>
				</div>

				<div class="activity_buy_count">
					<span>구매 횟수</span> <span>0회</span>
				</div>

				<div class="activity_refund_account">
					<span>환불 계좌</span> <span> <input type='text' value=""
						placeholder="은행" /> <input type='text' value="" placeholder="예금주"
						class="" /> <input type='text' value="" placeholder="계좌번호"
						class="" />
					</span>
				</div>

				<div class="activity_memo">
					<span>메모</span> <span> <textarea></textarea>
					</span>
				</div>
			...
			<!-- 변경사항 저장 버튼 -->
			<div class="save_button">변경사항 저장</div>
			<!-- 변경사항 젖저장 버튼 -->
	...
	<div class="invoice_window">
		...
		<div class="invoice_input">
			<input type='text' class="invoice_num" placeholder="송장번호" />
		</div>
		<img src="/resources/img/cross.png" class="invoice_close" />

		<div class="invoice_input">
			<input type='text' class="courier" placeholder="택배사" readonly />
			<ul class="invoice_ul">
	...
	<script src="/resources/js/admin/cs/cs.js?i=<%=i_order_new%>"></script>
