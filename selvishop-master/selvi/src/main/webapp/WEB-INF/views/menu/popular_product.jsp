<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>

<%@ page import="java.util.*"%>
<%
	Random r_index = new Random();
	int i_index = r_index.nextInt(1000);
%>

<link rel="shortcut icon"
	href="https://s3.ap-northeast-2.amazonaws.com/kimgeunho/favicon/favicon.ico">
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/menu/category.css"/>
	
<%@include file="../include/header.jsp"%>

<!-- 최상단, 최하단 이동 버튼 -->
<div id="up_down">
	<div class="up">
		<img
			src="https://s3.ap-northeast-2.amazonaws.com/supersell-selvi/selvi_shopping_mall_pc/%EC%9C%84%EB%A1%9C.png" />
	</div>
	<div class="down">
		<img
			src="https://s3.ap-northeast-2.amazonaws.com/supersell-selvi/selvi_shopping_mall_pc/%EC%95%84%EB%9E%98%EB%A1%9C.png" />
	</div>
</div>
<!-- 최상단, 최하단 이동 버튼 -->

<!-- 메인 슬라이드 -->
<!-- <div class="bxslider">
	<div>
		<img class="img1" src="/resources/img/누뉴580.jpg" />
	</div>

	<div>
		<img class="img2" src="/resources/img/삼각대580.jpg" />
	</div>
</div> -->
<!-- 메인 슬라이드 -->

<%
    List<Map<String, Object>> list = (List<Map<String, Object>>) request.getAttribute("list");

	// 상품 총 갯수
	int count = Integer.parseInt(String.valueOf(request.getAttribute("count"))); 
	
	// 한 페이지에서 보여줄 상품 개수 설정
	int pageSize = Integer.parseInt(String.valueOf(request.getAttribute("page_size"))); ;
	
	String pageNum = String.valueOf(request.getAttribute("page_num")); // 현재 페이지 위치
	
	if (pageNum == null) {
		pageNum = "1";
	}
	
	// 시작행을 계산하기   1...11...21...31...
	int currentPage = Integer.parseInt(pageNum);
	int startRow = (currentPage - 1) * pageSize + 1;
	
	// 끝행을 계산하기  10..... 20.....30...40... 
	int endRow = currentPage * pageSize;
%>
				
<!-- 메인 페이지 상품 목록-->
<div id="main_wrap">
	<!-- 상품 전체 목록 -->
	<div id="main_product_list">
		<!-- 제목 -->
		<div class="product_line_title">인기상품</div>
		<!-- 개수 선택 정렬 / 가격순, 최신순, 인기순 -->
		<div class="product_sort">
			<div class="product_sort_left">
				<span>Total : <b><%=count %></b> 개</span>
			</div>
			<div class="product_sort_right">
				<ul class="product_sort_right_ul">
					<li>높은 가격순</li>
					<li>낮은 가격순</li>
					<li>최신순</li>
					<li>인기순</li>
				</ul>
			</div>
		</div>
		<!-- 상품 4 * 4-->
		<%
			int divise = 4;

			for (int z = 0; z < list.size(); z++) {
				String prdt_price = String.valueOf(list.get(z).get("prdt_price"));
				String prdt_sale_price = String.valueOf(list.get(z).get("prdt_sale_price"));
				String prdt_name = String.valueOf(list.get(z).get("prdt_name"));
				int prdt_idx_pk = Integer.parseInt(String.valueOf(list.get(z).get("prdt_idx_pk")));

				StringBuffer prdt_price_sb = new StringBuffer(prdt_price);
				StringBuffer prdt_sale_price_sb = new StringBuffer(prdt_sale_price);

				String[] prdt_thumbnail = String.valueOf(list.get(z).get("prdt_thumbnail")).split("§");

				double sale_temp = Integer.parseInt(prdt_sale_price);
				double temp = Integer.parseInt(prdt_price);
				double prdt_price_percent = 100 - (100 * (sale_temp / temp));
		%>		
				
    	<div class="mb70 product_list_product"
    		onclick="window.location='/product_view?id=<%=prdt_idx_pk %>'" target="_parent">
			<figure class="product_list_figure">
				<div class="product_list_image">
					<div class="product_image">
					
					<%
						for (int a = 0; a < prdt_thumbnail.length; a++) {
								if (a == 1) {
					%>
					
						<img alt="이미지 제목" class="product_img"
							src="/resources/img/<%=prdt_thumbnail[a]%>">
					<%
						}
							}
					%>		
							
					</div>
					
				</div>
				<div class="product_list_name">
					<h5>
						<span class="product_name"><%=prdt_name%><%=prdt_idx_pk %></span>
					</h5>
				</div>
				<ul class="product_list_price">
					<li class="">
						<span class="product_price_cost"><%=prdt_price_sb.insert(1, ",")%>원</span>
					</li>
					<li class="">
						<span class="product_price_sale"><%=prdt_sale_price_sb.insert(1, ",")%>원</span>
						<span class="product_price_rate_discount"><%=Math.round(prdt_price_percent)%>%</span>
					</li>
				</ul>
			</figure>
		</div>
		
		<%
			}
		%>
	</div>
	
	<!-- 페이징 -->
	<div class="page">
		<ul>
			<%
				// 페이지 접근 형태 출력
				// <이전  1,2,3,4,5 다음>
				if (count != 0) {
					// 전체 페이지수 계산 => 게시판 글 50개 , 한페이지에 10개씩 보여줌  => 5페이지 
					//                게시판 글 77개 , 한페이지에 10개씩 보여줌  => 8페이지( 7+1 )
					int pageCount = count / pageSize + (count % pageSize == 0 ? 0 : 1);
					
					// 한 화면에 보여줄 페이지 번호의 개수 
					int pageBlock = 5;
					
					// 시작페이지 번호
					int startPage = ((currentPage - 1) / pageBlock) * pageBlock + 1;
					
					// 끝페이지 번호 
					int endPage = startPage + pageBlock - 1;
					if (endPage > pageCount) {
						endPage = pageCount;
					}
					
					// 이전
					if (startPage > pageBlock) {
			%>
						<li>
							<a href="category1?page_num=<%=startPage - pageBlock%>">
								<img src="https://s3.ap-northeast-2.amazonaws.com/supersell-selvi/shoppingmall/paging/btn_prev.png"
									style="width: 34px; height: 34px;" alt="이전 페이지 보기">
							</a>
						</li>
			<%
					}
					
					// 1~10,11~20,21~30,....
					for (int a = startPage; a <= endPage; a++) {
						if (a == currentPage) {
			%>
							<li class="on"><a href="category1?page_num=<%=a%>"><%=a%></a>
			<%
						} else {
			%>
							<li class="off">
								<a href="category1?page_num=<%=a%>"><%=a%></a>
							</li>
			<%
						}
					}
					
					// 다음
					if (endPage < pageCount) {
			%>
						<li>
							<a href="category1?page_num=<%=startPage + pageBlock%>">
								<img src="https://s3.ap-northeast-2.amazonaws.com/supersell-selvi/shoppingmall/paging/btn_next.png"
									style="width: 34px; height: 34px;" alt="다음 페이지 보기">
							</a>
						</li>
			<%
					}
				}
			%>
		</ul>
	</div>
</div>
<!-- 메인 페이지 상품 목록-->

<%@include file="../include/footer.jsp"%>

<script src="/resources/js/index/index.js?i=<%=i_index%>"></script>