...
// 동영상 파일 미리보기 기능
	  ...
	  function readAndPreview(file) {
          ...
          var reader = new FileReader(); 
 	      reader.onload = function(){	
 	    	 count = count + 1;
 	    	 $("#addProductImgBtn").append("<div data-id='"+count+"' class='product_img input_img'><img class='product_img_child' src='"+reader.result+"'/><img src='/resources/img/cross.png' class='product_img_cross' /></div>");
	      ...
	      if (file) {
	         ...
	         for(var i=0; i<file_length; i++){	  
	    	    if($("#productImageUploader")[0].files[i].name.match(/.*\.jpg|.*\.jpeg|.*\.png|.*\.gif/i)){  
	    	    	...
	    	    	alert("영상형식에 맞지않은 파일이 들어가있습니다. png, jpg, jpeg, gif 파일만 올려주십시오.");
	    	    	return;
	    	    }
	         } 
 ...
 $(document).on("click",".product_img_cross",function(){
    ...
    if($(this).parents(".input_img")){
    	
    	var index_in = $(this).parents(".input_img").index();
    	$(".product_img").eq(index).remove();
    	newFileList.splice(index_in,1);
    	
    }
    else if($(this).parents(".db_img")){
    	var index_in = $(this).parents(".db_img").index();
    	$(".product_img").eq(index).remove();
    }
...
// 태그 추가로 추가된 옵션칸 엑스표시 누르면 사라지게 해주는 기능
 ...
 $(document).on("click",".edit_icon",function(){
	 ...
	 var val = $("#shopCategoryList .trans-fast").eq(index).text();
 ...
 $("#saveShopProduct").click(function(e){
	  ...
	  // 대표이미지 src 값 구해서 특수문자로 이어놓기
	  $(".product_img_child").each(function(i){
		  
		  var val = $(".product_img_child img").eq(i).attr("src");
		  
		  //prdt_thumbnail = prdt_thumbnail +"§"+ val;
	  });
	 ...
	 $(".option_3").each(function(i){
	  ...
	  $("#shopCategoryList .trans-fast").each(function(){
		 prdt_category_large =  prdt_category_large + "§" + $(this).text();
	  });
      ...
      for(var z=0; z<$("#productImageUploader")[0].files.length; z++){
    	  prdt_thumbnail =  prdt_thumbnail + "" +  $("#productImageUploader")[0].files[z].name;
      }
