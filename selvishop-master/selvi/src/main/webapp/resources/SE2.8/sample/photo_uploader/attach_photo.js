	...
	try {
		if (!oNavigator.ie) {
			if (!!oNavigator.safari && oNavigator.version <= 5) {
				...
				// bSupportDragAndDropAPI = false; // 단일 사진 첨부
				bSupportDragAndDropAPI = true; // 다중 사진 첨부
			}
		} else {
			bSupportDragAndDropAPI = false;
		}
 ...
 * @return
 */
function html5Upload(ss) {
	var tempFile, sUploadURL;
    var json = {};
	// sUploadURL= 'file_uploader_html5.php'; //upload URL

	if(page_url == "admin_board"){
		sUploadURL = "/admin_board_image_add";
	}
	else if(page_url == "admin_product"){
		sUploadURL = "/admin_product_image_add";
	}
	else if(page_url == "product"){
		sUploadURL = "/product_image_add";
	}
	else if(page_url == "admin_product"){
		sUploadURL = "/product_image_add";
	}
...
function callFileUploader() {
	oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"), {
		sUrl : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php', // 샘플
																				// URL입니다.
